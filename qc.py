...
def remove_plaque_correlated_genes(adata: AnnData) -> AnnData:
    """
    Find genes correlated with plaque overlap.
    Returns: correlation_df, genes_to_remove
    """
    ...
    threshold_r2 = neg_probes.iloc[1]['R2']
    threshold_gene = neg_probes.iloc[1]['gene']

    print(f'Using R² threshold from 2nd highest negative probe:')
    ...
    all_neg_probes = neg_probes['gene'].tolist()
    genes_to_remove.extend(all_neg_probes)

    print(f'Removing {len(genes_to_remove)} genes and {len(all_neg_probes)} neg probes from AnnData...')
